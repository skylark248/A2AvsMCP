...
- [x] **HEAT-01**: `HardnessFailureHeatmap.tsx` renders rows = HardnessType, columns = lane; cell shows dominant_tag color + icon + pattern fill + recovery rate (e.g., `12/15`); cells keyboard-focusable; "directional · n=3 tasks · v1" pill in `secondary.main`
- [x] **HEAT-02**: Heatmap legend strip always visible (5 inline pills); footer shows model · seed · pinned task IDs
